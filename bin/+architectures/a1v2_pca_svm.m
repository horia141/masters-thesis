classdef a1v2_pca_svm < architecture
    ...
    methods (Access=public)
        function [obj] = a1v2_pca_svm(train_dataset_plain,pca_kept_energy,svm_kernel_type,svm_kernel_param)
            ...
            assert(tc.scalar(svm_kernel_type) && tc.string(svm_kernel_type) && ...
                   (strcmp(svm_kernel_type,'linear') || strcmp(svm_kernel_type,'rbf') || strcmp(svm_kernel_type,'poly')));
            assert((strcmp(svm_kernel_type,'linear') && ...
                     (~exist('svm_kernel_param','var') || ...
                       (tc.scalar(svm_kernel_param) && tc.number(svm_kernel_param) && (svm_kernel_param == 0)))) || ...
                   (strcmp(svm_kernel_type,'rbf') && ...
                     (tc.scalar(svm_kernel_param) && tc.number(svm_kernel_param) && (svm_kernel_param > 0))) || ...
                   (strcmp(svm_kernel_type,'poly') && ...
                     (tc.scalar(svm_kernel_param) && tc.natural(svm_kernel_param) && (svm_kernel_param > 0))));
                 
            if strcmp(svm_kernel_type,'linear')
                svm_kernel_param_t = 0;
            elseif strcmp(svm_kernel_type,'rbf')
                svm_kernel_param_t = svm_kernel_param;
            elseif strcmp(svm_kernel_type,'poly')
                svm_kernel_param_t = svm_kernel_param;
            else
                assert(false);
            end
            ...
            classifier = classifiers.one_vs_one(train_dataset_plain_1,...
                                                @classifiers.svm,...
                                                {svm_kernel_type svm_kernel_param_t});
            
            obj = obj@architecture(train_dataset_plain.subsamples(1),{t_pca},classifier);
            obj.pca_kept_energy = pca_kept_energy;
            obj.svm_kernel_type = svm_kernel_type;
            obj.svm_kernel_param = svm_kernel_param_t;
        end
    end
    
    methods (Static,Access=public)
        function test(display)
            fprintf('Testing "architecture.a1v2_pca_svm".\n');
            ...
            ar = architectures.a1v2_pca_svm(s,0.90,'linear');
            ...
            ar = architectures.a1v2_pca_svm(s_tr,0.90,'linear');
            ...
            ar = architectures.a1v2_pca_svm(s_tr,0.90,'rbf',0.5);
