...
do_task(
    ...
    for (ii = 0; ii < task_info_count; ii++) {
        correlation_order(task_info[ii].o_coeffs_pr,task_info[ii].o_coeffs_ir,
                          global_info->geometry,global_info->word_count,global_info->dict,global_info->dict_transp,global_info->dict_x_dict_transp,
                          global_info->coeff_count,global_info->modulator,task_info[ii].observation,coding_tmps);
    }
...
mexFunction(
    ...
    global_info.modulator = modulator;
