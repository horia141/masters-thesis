...
RESULTS_PATH = '../../explogs/baseline/norbsmall/results_1.mat';
...
fprintf('Experiment "Baseline - NORBSmall"\n');
...
[s_tr,ci_tr] = dataset.load('../../data/norbsmall.train.mat');

fprintf('%d observations.\n',dataset.count(s_tr));

fprintf('  Loading test sample ... ');

[s_ts,ci_ts] = dataset.load('../../data/norbsmall.test.mat');
...
for coder_idx = 1:length(param_list_coder)
    ...
    for coder_rep_idx = 1:CODER_REP_COUNT
        ...
        s_coder_useful_coded = sparse(dataset.flatten_image(s_coder_useful(1:4:end,:,1,:)));
        s_ts_coded = sparse(dataset.flatten_image(s_ts(1:4:end,:,1,:)));
